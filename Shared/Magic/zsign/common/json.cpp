 ...
 *
 * Backdoor App Signer is proprietary software.
 * You may not use, modify, or distribute it except as expressly permitted under the terms of the Proprietary
 * Software License.
 */
...
JValue::JValue(int val) : m_eType(E_INT) { m_Value.vInt64 = val; }

JValue::JValue(int64_t val) : m_eType(E_INT) { m_Value.vInt64 = val; }

JValue::JValue(bool val) : m_eType(E_BOOL) { m_Value.vBool = val; }

JValue::JValue(double val) : m_eType(E_FLOAT) { m_Value.vFloat = val; }

JValue::JValue(const char *val) : m_eType(E_STRING) { m_Value.vString = NewString(val); }

JValue::JValue(const string &val) : m_eType(E_STRING) { m_Value.vString = NewString(val.c_str()); }

JValue::JValue(const JValue &other) { CopyValue(other); }
...
void JValue::clear() { Free(); }

bool JValue::isInt() const { return (E_INT == m_eType); }

bool JValue::isNull() const { return (E_NULL == m_eType); }

bool JValue::isBool() const { return (E_BOOL == m_eType); }

bool JValue::isFloat() const { return (E_FLOAT == m_eType); }

bool JValue::isString() const { return (E_STRING == m_eType); }

bool JValue::isArray() const { return (E_ARRAY == m_eType); }

bool JValue::isObject() const { return (E_OBJECT == m_eType); }
...
JValue::operator const char *() const { return asCString(); }

JValue::operator int() const { return asInt(); }

JValue::operator int64_t() const { return asInt64(); }

JValue::operator double() const { return asFloat(); }

JValue::operator string() const { return asCString(); }

JValue::operator bool() const { return asBool(); }
...
JValue &JValue::operator[](int index) { return (*this)[(size_t)(index < 0 ? 0 : index)]; }

const JValue &JValue::operator[](int index) const { return (*this)[(size_t)(index < 0 ? 0 : index)]; }

JValue &JValue::operator[](int64_t index) { return (*this)[(size_t)(index < 0 ? 0 : index)]; }

const JValue &JValue::operator[](int64_t index) const { return (*this)[(size_t)(index < 0 ? 0 : index)]; }
...
JValue &JValue::operator[](const string &key) { return (*this)[key.c_str()]; }

const JValue &JValue::operator[](const string &key) const { return (*this)[key.c_str()]; }
...
JValue &JValue::at(int index) { return (*this)[index]; }

JValue &JValue::at(size_t index) { return (*this)[index]; }

JValue &JValue::at(const char *key) { return (*this)[key]; }
...
bool JValue::read(const string &strdoc, string *pstrerr) { return read(strdoc.c_str(), pstrerr); }
...
bool JValue::push_back(int val) { return push_back(JValue(val)); }

bool JValue::push_back(bool val) { return push_back(JValue(val)); }

bool JValue::push_back(double val) { return push_back(JValue(val)); }

bool JValue::push_back(int64_t val) { return push_back(JValue(val)); }

bool JValue::push_back(const char *val) { return push_back(JValue(val)); }

bool JValue::push_back(const string &val) { return push_back(JValue(val)); }
...
bool JValue::push_back(const char *val, size_t len) { return push_back(JValue(val, len)); }
...
bool JValue::isData() const { return (E_DATA == m_eType); }

bool JValue::isDate() const { return (E_DATE == m_eType); }
...
char JReader::GetNextChar() { return (m_pCur == m_pEnd) ? 0 : *m_pCur++; }
